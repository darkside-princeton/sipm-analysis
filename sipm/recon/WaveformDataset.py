...
class WaveformDataset: 
    ...
    def read_calibration_h5(self, filename):
        ...
        """
        self.calib_df = pd.read_hdf(filename, key=f'/{self.volt}V')
        if 'Qpeak' in self.calib_df and 'Qap' in self.calib_df and 'DiCT' in self.calib_df:
    ...
    def process_laser_pulses(self, header=True, num_events=1e9):
        ...
        for i in self.channels:
            ...
            self.ch[i].get_integral(length_us=[0.75]) # 0.75us integral
        self.clear()

...
def main():
    ...
    d = wfd.WaveformDataset(path=args.file_dir, samples=5000, channels=np.arange(8),trig=950)
    ...
    for i in d.channels:
        ...
        d.ch[i].output['fired'] = d.ch[i].output['integral_10p00us']>0.5*d.calib_df['cn_corrected_gain'][i]
        d.ch[i].get_max()
    d.output['nch'] = np.zeros_like(d.ch[0].output['fired'])
    d.output['nch_fs'] = np.zeros_like(d.ch[0].output['fired'])
    d.output['nch_nofs'] = np.zeros_like(d.ch[0].output['fired'])
    for i in d.channels:
        d.output['nch'] = d.output['nch'] + d.ch[i].output['fired'].astype(int)
    for i in [0,3,5,6]:
        d.output['nch_fs'] = d.output['nch_fs'] + d.ch[i].output['fired'].astype(int)
    for i in [1,2,4,7]:
        d.output['nch_nofs'] = d.output['nch_nofs'] + d.ch[i].output['fired'].astype(int)
    d.get_total_pe(length_us=10)
    d.get_fprompt(tprompt=[0.3],channels=np.arange(8),t_all=10)
    d.get_fprompt(tprompt=[0.3],channels=np.array([0,3,5,6]),t_all=10)
    d.get_fprompt(tprompt=[0.3],channels=np.array([1,2,4,7]),t_all=10)
    d.clear()
    ...
    # Save data to HDF5
    io.save(script='scintillation_pulse_liq7')

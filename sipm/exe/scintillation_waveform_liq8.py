...
def main():
    ...
    d = wfd.WaveformDataset(path=args.file_dir,channels=[1,2,4,7],trig=700)
    ...
    for i in d.channels:
        ...
        d.ch[i].get_integral(length_us=[0.3,7]) # 0.3us for Fprompt analysis
        d.ch[i].get_max()
    d.get_total_pe(length_us=7)
    d.get_fprompt(tprompt=[0.3],channels=[1,2,4,7],t_all=7)
    ...
    for i in d.channels:
        ...
        cut = cut & (np.array(d.ch[i].output['baseline_rms'])<2.0)
    ...
    for i in d.channels:
        ...
        d.ch[i].output.pop('integral_7p00us')
    ...
    # Save data to HDF5
    io.save(script='scintillation_waveform_liq8')

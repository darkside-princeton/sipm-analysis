...
def main():
    ...
    for i in d.channels:
        ...
        d.ch[i].get_integral(length_us=[0.3,7]) # <=0.5us for Fprompt analysis
        d.ch[i].get_max()
    d.get_total_pe(length_us=7)
    d.get_fprompt(tprompt=[0.3],channels=d.channels,t_all=7)
    d.clear()
    ...
    # Save data to HDF5
    io.save(script='scintillation_pulse_liq8')

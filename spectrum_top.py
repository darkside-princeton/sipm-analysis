...
for i,v in enumerate(bias):
    ...
    print('Top {}V {} events loaded. Cut fraction {:.2f}%'.format(v, nev, (1-nev_cut/nev)*100))
...
for i in range(len(ds1120top)):
    hg,hgx = np.histogram(ds1120top[i].summed_integral_pe, bins=np.linspace(xmin,xmax,nbins+1))
    ...
    hg = hg/norm_
    hist_gamma_top_err[i] = hist_gamma_top_err[i]/norm_
    ...
    hb,hbx = np.histogram(ds1118top_bkg[i].summed_integral_pe, bins=np.linspace(xmin,xmax,nbins+1))
    hist_bkg_top_err.append(np.sqrt(hb))
    norm_ = np.sum(hb[int((bkg_boundary[i]-xmin)/bin_width):])
    hb = hb/norm_
    hist_bkg_top_err[i] = hist_bkg_top_err[i]/norm_
...
for iv,volt in enumerate(bias):
    ...
    plt.ylim(ymin, ymax)
    plt.savefig('cs137_top_{}v_fit.png'.format(bias[iv]))
    ...
    plt.ylabel(r'$\alpha$')
    plt.minorticks_on()
    plt.savefig('chisq_top_{}v.png'.format(bias[iv]))
